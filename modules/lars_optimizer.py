...
def init_optim(model: torch.nn.Module, args: Dict):
    ...
    if args['base_optimizer'] == "Adam":
        base_optim = Adam(model.parameters(), lr=lr, weight_decay=args['weight_decay'])
    ...
    elif args['base_optimizer'] == 'OldLARS':
        base_optim = OldLARS(
            model.parameters(), 
            lr, 
            momentum=args['momentum'], 
    ...
    if args['lars'] and args['base_optimizer'] != 'OldLARS':
        optimizer = LARS(optimizer=base_optim, eps=args['eps'], trust_coef=args['trust_coef'])
    else:
